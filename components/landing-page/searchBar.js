...
const SearchBar = ({ handleSearchedCoordinate, nameResult }) => {
  ...
  const handleSelect = (lat, lon, name) => {
    handleSearchedCoordinate([lat, lon]);
    setQuery(name)
    const nameList = name.split(",")
    nameResult(nameList[0])
    setResults([]);
  };
          ...
          <ul className="mx-5 bg-white p-3">
            {results.map((item, index) => (
              <li
                ...
                onClick={() => handleSelect(item.lat, item.lon, item.display_name)}
              >
                {item.display_name}
              </li>
            ))}
