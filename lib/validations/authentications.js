...
export const raiseRequestSchema = yup.object().shape({
  selectedProperty: yup
    .string()
    .required("Select a property"),
  ...
  estimatedPrice: yup
    ...
    .min(1, "Estimated price must be greater than 0")
    .required("Estimated price is required"),
  urlsList: yup
    .string()
    .test(
      "is-valid-urls",
      "Urls added incorrectly",
      (value) =>
        !value || /^(https?:\/\/[^\s,]+)(,[^\s,]+)*$/.test(value)
    ),
});

export const withdrawalManagementSchema = yup.object().shape({
  amount: yup
    .number()
    .typeError("Withdrawal amount must be a number")
    .positive("Withdrawal amount must be greater than 0") // Ensures the value is greater than 0
